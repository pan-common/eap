
package com.taiji.eap.biz.zxjg.bean;

import com.taiji.eap.biz.jcxmjg.bean.Jcxmjg;
import com.taiji.eap.common.base.BaseModel;

import java.util.List;

public class Zxjg extends BaseModel{
    private Long zxjgId;//主键
    private String qybh;//企业编号
    private String jcdfl;//监测点分类
    private String jcdbh;//监测点编号
    private String jcdmc;//监测点名称
    private String sj;//时间
    private String klwnd;//颗粒物浓度
    private String klwzsnd;//颗粒物折算浓度
    private String klwzl;//颗粒物总量
    private String eyhlnd;//二氧化硫浓度
    private String eyhlzsnd;//二氧化硫折算浓度
    private String eyhlzl;//二氧化硫总量
    private String dyhwnd;//氮氧化物浓度
    private String dyhwzsnd;//氮氧化物折算浓度
    private String dyhwzl;//氮氧化物总量
    private String bgll;//标杆流量
    private String yl;//氧量
    private String yw;//烟温
    private String hsl;//含湿量
    private String gzxx;//故障信息
    private String bz;//备注
    private String sfyz;//是否运行

    private List<Jcxmjg> jcxmjgs;//监测项目结果

    public Long getZxjgId() {
        return zxjgId;
    }

    public void setZxjgId(Long zxjgId) {
        this.zxjgId = zxjgId;
    }


    public String getQybh() {
        return qybh;
    }

    public void setQybh(String qybh) {
        this.qybh = qybh;
    }


    public String getJcdfl() {
        return jcdfl;
    }

    public void setJcdfl(String jcdfl) {
        this.jcdfl = jcdfl;
    }


    public String getJcdbh() {
        return jcdbh;
    }

    public void setJcdbh(String jcdbh) {
        this.jcdbh = jcdbh;
    }


    public String getJcdmc() {
        return jcdmc;
    }

    public void setJcdmc(String jcdmc) {
        this.jcdmc = jcdmc;
    }


    public String getSj() {
        return sj;
    }

    public void setSj(String sj) {
        this.sj = sj;
    }


    public String getKlwnd() {
        return klwnd;
    }

    public void setKlwnd(String klwnd) {
        this.klwnd = klwnd;
    }


    public String getKlwzsnd() {
        return klwzsnd;
    }

    public void setKlwzsnd(String klwzsnd) {
        this.klwzsnd = klwzsnd;
    }


    public String getKlwzl() {
        return klwzl;
    }

    public void setKlwzl(String klwzl) {
        this.klwzl = klwzl;
    }


    public String getEyhlnd() {
        return eyhlnd;
    }

    public void setEyhlnd(String eyhlnd) {
        this.eyhlnd = eyhlnd;
    }


    public String getEyhlzsnd() {
        return eyhlzsnd;
    }

    public void setEyhlzsnd(String eyhlzsnd) {
        this.eyhlzsnd = eyhlzsnd;
    }


    public String getEyhlzl() {
        return eyhlzl;
    }

    public void setEyhlzl(String eyhlzl) {
        this.eyhlzl = eyhlzl;
    }


    public String getDyhwnd() {
        return dyhwnd;
    }

    public void setDyhwnd(String dyhwnd) {
        this.dyhwnd = dyhwnd;
    }


    public String getDyhwzsnd() {
        return dyhwzsnd;
    }

    public void setDyhwzsnd(String dyhwzsnd) {
        this.dyhwzsnd = dyhwzsnd;
    }


    public String getDyhwzl() {
        return dyhwzl;
    }

    public void setDyhwzl(String dyhwzl) {
        this.dyhwzl = dyhwzl;
    }


    public String getBgll() {
        return bgll;
    }

    public void setBgll(String bgll) {
        this.bgll = bgll;
    }


    public String getYl() {
        return yl;
    }

    public void setYl(String yl) {
        this.yl = yl;
    }


    public String getYw() {
        return yw;
    }

    public void setYw(String yw) {
        this.yw = yw;
    }


    public String getHsl() {
        return hsl;
    }

    public void setHsl(String hsl) {
        this.hsl = hsl;
    }


    public String getGzxx() {
        return gzxx;
    }

    public void setGzxx(String gzxx) {
        this.gzxx = gzxx;
    }


    public String getBz() {
        return bz;
    }

    public void setBz(String bz) {
        this.bz = bz;
    }


    public String getSfyz() {
        return sfyz;
    }

    public void setSfyz(String sfyz) {
        this.sfyz = sfyz;
    }

    public List<Jcxmjg> getJcxmjgs() {
        return jcxmjgs;
    }

    public void setJcxmjgs(List<Jcxmjg> jcxmjgs) {
        this.jcxmjgs = jcxmjgs;
    }
}
