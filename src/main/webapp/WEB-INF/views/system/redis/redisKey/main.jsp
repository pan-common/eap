...
<script type="text/javascript">
    ...
    layui.use([ 'layer', 'form' ], function(layer, form) {
        ...
        $("#addBtn").click(function () {
            showModel("新增","${pageContext.request.contextPath}/sysResource/link?url=system/redis/redisKey/form&keyId=0","550px","550px");
        });
        $("#showTreeView").click(function () {
            showModel("显示树","${pageContext.request.contextPath}/sysResource/link?url=system/redis/redisKey/treeView","550px",$(window).height())
        });
        $("#showZTree").click(function () {
            showModel("显示树","${pageContext.request.contextPath}/sysResource/link?url=system/redis/redisKey/zTree","550px",$(window).height())
        });
        //弹出录入框
        ...
        $('#bootstrapTable').bootstrapTable({
            url:"${pageContext.request.contextPath}/redisKey/list",
            method:'GET',
                ...
                },
                {
                    ...
                    events : {
                        ...
                        'click .edit' : function(e, value, row, index) {
                            ...
                            showModel("编辑","${pageContext.request.contextPath}/sysResource/link?url=system/redis/redisKey/form&keyId="+row.keyId,"550px","550px");
                        },
    ...
    function del(keyId) {
        layer.confirm("删除数据不可恢复，请确认？",{
            ...
            $.post('${pageContext.request.contextPath}/redisKey/delete',
                    {keyId : keyId},
    ...
    function loadPath() {
        $.get('${pageContext.request.contextPath}/redisKey/getPath/',
                {
                    keyId : currentId
                }, function(data, status) {
                    if (status == "success") {
                        if (data.body.resultCode == "0") {
                            ...
                            for (var i = 0; i < result.length; i++) {
                                ...
                                +result[i].keyId+'">'+ result[i].keyName+'</a></li>';
                                $(".breadcrumb").append(html);
                                $("a[name=" + result[i].keyId + "]").bind("click", {
                                    index : i
                                }, clickHandler);
                            }
                            function clickHandler(event) {
                                var i = event.data.index;
                                currentId = result[i].keyId;
                                refreshTable();
                                loadPath();
                            }
                        }
