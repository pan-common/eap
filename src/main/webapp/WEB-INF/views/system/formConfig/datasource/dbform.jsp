...
<form id="form" class="layui-form" style="margin-top: 20px" lay-filter="form">
    ...
    <div class="layui-form-item">
        <div class="layui-input-block" style="margin-right: 10px">
            <button id="connectBtn" type="button" class="layui-btn" lay-submit lay-filter="connectBtn">测试连接</button>
            <button id="submitBtn" class="layui-btn layui-btn-disabled" lay-submit lay-filter="submitBtn">提交</button>
...
<script type="text/javascript">
    ...
    layui.use(['form'],function () {
        ...
        $("#connectBtn").click(function () {
            setTimeout("parent.layer.msg('网络错误', {icon: 5});",5000)
            $.post(baseServerUrl+"dataSource/connectTest",
                $("#form").serializeArray(),
                function (data,status) {
                    if(status=="success"){
                        if(data.body.resultCode=="0"){
                            $("#submitBtn").removeClass("layui-btn-disabled");
                            parent.layer.msg(data.body.entity, {icon: 1});
                        }else {
                            parent.layer.msg(data.body.content, {icon: 5});
                        }
                    }else {
        ...
        form.on('select(datasourceType)',function (data) {
            var value = data.value;
            $.get(baseServerUrl+"dictionary/getDictionaryByKey",{
                keystone:value,
                parentId:78
            },function (data,status) {
                if(status=='success'){
                    if(data.body.resultCode=="0"){
                        $("input[name='driverClassName']").val(data.body.entity.param1);
                        $("input[name='url']").val(data.body.entity.param2)
                    }else {
                        parent.layer.msg(data.body.resultContent, {icon: 5});
                    }
                }else {
