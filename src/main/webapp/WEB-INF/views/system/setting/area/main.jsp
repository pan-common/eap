...
<script type="text/javascript">
    ...
    function del(areaId) {
        layer.confirm("删除数据不可恢复，请确认？",{
            ...
            $.post('${pageContext.request.contextPath}/area/delete',
                    {areaId : areaId},
    ...
    function loadPath() {
        $.get('${pageContext.request.contextPath}/area/getPath/',
                {
                    areaId : currentId
                }, function(data, status) {
                    if (status == "success") {
                        if (data.body.resultCode == "0") {
                            ...
                            for (var i = 0; i < result.length; i++) {
                                ...
                                +result[i].areaId+'">'+ result[i].areaName+'</a></li>';
                                $(".breadcrumb").append(html);
                                $("a[name=" + result[i].areaId + "]").bind("click", {
                                    index : i
                                }, clickHandler);
                            }
                            function clickHandler(event) {
                                var i = event.data.index;
                                currentId = result[i].areaId;
                                refreshTable();
                                loadPath();
                            }
                        }
