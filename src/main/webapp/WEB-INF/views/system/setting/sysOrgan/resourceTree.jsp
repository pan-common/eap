...
<script type="text/javascript">
    ...
    layui.use([ 'layer', 'form' ], function(layer, form) {
        ...
        $("#submit").click(function () {
            ...
            $.post(baseServerUrl+"sysResource/saveOrganResource",{
                organId:organId,
        ...
        var mainSetting = {
            async:{
                ...
                type: "get",
                dataType: "json",
                url: "${pageContext.request.contextPath}/sysResource/treeView",
                otherParam: {
                    parentId:0
                }
            },
            ...
            callback:{
                onAsyncSuccess:function (event, treeId, treeNode, data) {
                    if(data.body.resultCode=="0"){
                        ...
                        $.get(baseServerUrl+"sysResource/getResourceIdsByOrganId",{
                            organId:organId
                        },function (data, status) {
                            if(status=="success"){
                                if(data.body.resultCode=="0"){
                                    for (var i = 0; i < data.body.entity.length; i++) {
                                        var resourceId = data.body.entity[i];
                                        var treeNode = mainTree.getNodeByParam("resourceId",resourceId);
        ...
        function showSelectTree() {
            ...
            if(mainNodes.length>0) {
                for (var i = 0; i < mainNodes.length; i++) {
                    if(i==0){
                        resourceIds+=mainNodes[i].resourceId;
                    }else {
                        resourceIds+=","+mainNodes[i].resourceId;
                    }
                }
            }
            $.get(baseServerUrl+"sysResource/getTreeByResourceIds",{
                resourceIds:resourceIds
            },function (data,status) {
                if(status=="success"){
                    if(data.body.resultCode=="0"){
